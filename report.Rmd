...
**Summary:** To assess whether habitual coffee consumption is related to blood pressure and pulse, we compared SBP, DBP, and resting pulse between adults with and without coffee-drinking habits using violin plots, mean plots with 95% CIs, and ridgeline density plots. SBP and DBP distributions were highly similar in the two groups; coffee drinkers showed only a slight upward shift in SBP of a few mmHg, while DBP means were virtually identical and confidence intervals overlapped substantially. Pulse distributions also overlapped, with coffee drinkers trending toward a minorly higher resting pulse, but the separation between groups was small. From a cardiovascular-risk perspective, mean SBP and DBP in both groups remained in the normotensive to mildly elevated range, and group differences were far below clinical thresholds for hypertension. Thus, these unadjusted cross-sectional comparisons do not provide strong evidence that habitual coffee consumption is associated with meaningful chronic elevations in blood pressure or resting heart rate, and potential confounding by age, BMI, smoking, medication use, and other lifestyle factors remains an important concern.
...
**Summary:** To examine whether habitual coffee consumption is related to sleep, we compared self-reported sleep duration between adults with and without coffee habits using violin plots and paired weekday–weekend line plots. Average nightly sleep time was very similar in both groups, with most participants reporting about 7–8 hours of sleep and substantial overlap in the distributions; coffee drinkers appeared to sleep only slightly fewer hours on average. Paired plots showed that people in all groups slept more on weekends than on weekdays, and the size of this weekday–weekend “catch-up” was nearly identical for coffee drinkers and non-drinkers. Overall, these descriptive results do not suggest that regular coffee consumption is associated with large reductions in sleep duration or with a different weekday–weekend sleep pattern, though the estimates are based on self-reported sleep and unadjusted for important factors such as work schedule, stress, and mental health.
...
<br> 
**Summary:** Among adults with complete diabetes biomarker data (N = 2439), mean fasting glucose and HbA1c were very similar in coffee drinkers and non-drinkers, while fasting insulin was clearly lower in the coffee group. Glucose averaged 108.9 vs 109.5 mg/dL and HbA1c 5.71% vs 5.76% (mean differences 0.58 mg/dL and 0.04 percentage points; 95% CIs include 0; p = 0.75 and 0.43), indicating no meaningful difference in average glycemic control. In contrast, fasting insulin was 17.35 μU/mL in non-drinkers and 12.12 μU/mL in coffee drinkers, a difference of –5.23 μU/mL (95% CI: –7.89, –2.56; p ≈ 1.3×10⁻⁴), suggesting substantially lower insulin levels among those with a coffee habit. This pattern is compatible with better insulin sensitivity at similar glucose levels, but the results are cross-sectional and unadjusted for key confounders (e.g., age, BMI, smoking, diet, physical activity), so they should be viewed as descriptive rather than causal and as motivation for more detailed regression analyses.
...
**Summary:** Among adults with complete cholesterol data, we compared HDL, LDL, and total cholesterol between those with and without a coffee habit using violin plots and two-sample t-tests. Coffee drinkers had higher mean levels of all three markers: HDL was clearly higher, while increases in LDL and total cholesterol were modest (≈3–6 mg/dL) and only weakly supported statistically. From a CVD-risk perspective, this pattern is mixed but not clearly adverse—higher HDL is generally protective, and the small elevations in LDL and total cholesterol are minor relative to clinical thresholds for dyslipidemia. As with our other outcomes, these results are cross-sectional and unadjusted for key factors such as diet, BMI, smoking, alcohol, physical activity, and lipid-lowering therapy, so they should not be interpreted as causal effects of coffee. Instead, they suggest that coffee drinkers in NHANES tend to have higher HDL with slightly higher total cholesterol, a profile whose net impact on cardiovascular risk remains uncertain and motivates more detailed adjusted analyses.
...
```{r Blood pressure regression, include=FALSE}
...
model_sbp %>% 
  ...
  kable(
    format = "html",
    caption = "Linear Regression: SBP on Caffeine Intake"
  ) %>%
...
model_dbp %>% 
  ...
  kable(
    format = "html",
    caption = "Linear Regression: DBP on Caffeine Intake"
  ) %>%
  kable_styling(full_width = FALSE)
```
...
model_sleep %>%
  ...
  kable(
    format = "html",
    caption = "Linear Regression: Sleep Duration on Caffeine Intake"
  ) %>%
  kable_styling(full_width = FALSE)
```
...
```{r Diabetes regression model, include=FALSE}
...
model_glyco %>%
  ...
  kable(
    format = "html",
    caption = "Regression: Glycohemoglobin (%) on Caffeine Intake"
  ) %>%
  kable_styling(full_width = FALSE)
...
model_insulin %>%
  ...
  kable(
    format = "html",
    caption = "Regression: Insulin (µU/mL) on Caffeine Intake"
  ) %>%
  kable_styling(full_width = FALSE)
...
model_glucose %>%
  ...
  kable(
    format = "html",
    caption = "Regression: Blood Glucose (mg/dL) on Caffeine Intake"
  ) %>%
  kable_styling(full_width = FALSE)
```
...
```{r CVD regression model, include=FALSE}
...
- Total Cholesterol ~ Caffeine Intake
...
model_chlesterol %>%
  ...
  kable(
    format = "html",
    caption = "Regression: Total Cholesterol (mg/dL) on Caffeine Intake"
  ) %>%
  kable_styling(full_width = FALSE)
```
...
model_hdl %>%
  ...
  kable(
    format = "html",
    caption = "Regression: HDL Cholesterol (mg/dL) on Caffeine Intake"
  ) %>%
  kable_styling(full_width = FALSE)
```
...
model_ldl %>%
  ...
  kable(
    format = "html",
    caption = "Regression: LDL Cholesterol (mg/dL) on Caffeine Intake"
  ) %>%
  kable_styling(full_width = FALSE)
```
